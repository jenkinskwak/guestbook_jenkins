...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="jenkinskwak/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/jenkinskwak/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage ('JMeter LoadTest') {
            steps { 
                sh '~/kwak/lab/sw/jmeter/bin/jmeter.sh -j jmeter.save.saveservice.output_format=xml -n -t src/main/jmx/guestbook_loadtest.jmx -l loadtest_result.jtl' 
